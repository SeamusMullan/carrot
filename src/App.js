...
function App() {
  ...
  const handleVegetableClick = (e) => {
    ...
    const vegStats = vegetableStats[currentVegetable] || { perClick: 1, perSecond: 0 };
  ...
  const loadGame = async () => {
    ...
    try {
      ...
      if (savedData) {
        ...
        if (data.playerVegetables) {
          setPlayerVegetables(data.playerVegetables);
        }
        
        if (data.vegetableStats) {
          setVegetableStats(data.vegetableStats);
        }
          ...
          <>
            ...
            <div className="main-section">
              <div className="carrot-field">
                <button className="carrot-btn" onClick={handleVegetableClick}>
                  <span className="carrot-emoji">{getCurrentVegetableEmoji()}</span>
                </button>
            ...
            <div className="shop-section">
              ...
              <div className="power-ups-tabs">
                <button 
                  className={`tab-button ${currentVegetable === 'universal' ? 'active' : ''}`}
                ...
                {Object.keys(playerVegetables).map(veg => (
                  <button 
                    key={veg}
                    className={`tab-button ${currentVegetable === veg ? 'active' : ''}`}
                    onClick={() => setCurrentVegetable(veg)}
                  >
                    {getVegetableName(veg)} {vegetableStats[veg].perClick > 1 || vegetableStats[veg].perSecond > 0 ? '⭐' : ''}
                  </button>
                ))}
              </div>
              ...
              <div className="power-ups">
                {powerUps
                  ...
                  .map(powerUp => (
                    <div 
                      key={powerUp.id} 
                      className={`power-up ${playerVegetables.carrot >= calculateCost(powerUp.baseCost, powerUp.count) ? 'affordable' : 'not-affordable'}`}
          ...
          <div className="current-veg-info">
            ...
            <div className="veg-details">
              ...
              <div className="veg-stats">
                <p>Per click: {formatNumber(vegetableStats[currentVegetable]?.perClick || 0)}</p>
                <p>Per second: {formatNumber(vegetableStats[currentVegetable]?.perSecond || 0)}</p>
                <p>Inventory: {formatNumber(playerVegetables[currentVegetable] || 0)} {getCurrentVegetableEmoji()}</p>
              </div>
            </div>
          </div>
              ...
              <div key={veg} className="inventory-item">
                <span className="inventory-emoji">{
                  { carrot: '🥕', potato: '🥔', tomato: '🍅', 
                    lettuce: '🥬', corn: '🌽', onion: '🧅' }[veg]
                }</span>
                <span className="inventory-count">{formatNumber(playerVegetables[veg] || 0)}</span>
              </div>
            ))}
          </div>
        </div>
